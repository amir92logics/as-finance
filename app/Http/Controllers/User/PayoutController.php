...
class PayoutController extends Controller
{
    ...
    public function confirmPayout(Request $request, $trx_id)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            $user = Auth::user();
            if ($user->{$payout->balance_type} < $payout->net_amount_in_base_currency){
                return back()->with('error', 'Insufficient Balance to deducted.');
            }
            updateBalance($payout->user_id, $payout->net_amount_in_base_currency, 0,$payout->balance_type); //update user balance
    ...
    public function paystackPayout(Request $request, $trx_id)
    {
        ...
        if (optional($payout->payoutMethod)->is_automatic == 1 && $basicControl->automatic_payout_permission && $request->automatic_payout_permission) {
