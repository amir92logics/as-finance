...
{
    public function validationCheck($amount, $gateway, $currency, $cryptoCurrency = null,$paymentType = null)
    {
        ...
        try {
            ...
            if (999 < $gateway->id) {
                ...
                $isCrypto = (checkTo($gateway->currencies, $currency) == 1) ? true : false;
            }
            ...
            if ($isCrypto) {
                ...
                if ($selectedCurrency !== false) {
                    ...
                    return [
                        'status' => false,
                        'message' => "Please choose the currency you'd like to use for payment"
                    ];
                }
            }
            ...
            $data['currency'] = ($gateway->currency_type == 1) ? ($currencyInfo->name ?? $currencyInfo->currency) : $cryptoCurrency ?? 'USD';
